...
// My life for the lord, and scores
var score = 0;
var allowedLives = 3;
...
//  FOKIN IVNADERS rows and columnds
...
function gameLoop(){
  ...
  //game won by conquering the battlefield
  if(aliveInvaders == 0){
    gameRunning=false;
    drawGameOver("гребаная сука, good job!");
    drawBottomHelper();
    return false;
  }
  ctx.clearRect(0,0,canvas.width,canvas.height);
...
// PENDENGAR ACARA also known as event listeners
...
// PENGENDALI also known as handlers
...
function moveInvaderBullets(){
  for(let i = 0 ; i < invaderBulletsArray.length; i++){
    ...
    //check if bullet dissapera
    if(iB.y > canvas.height){
      invaderBulletsArray.splice(i,1);
    }
    //check if game over by getting hit
    if(
      iB.x > tankX &&
